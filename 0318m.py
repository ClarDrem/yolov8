...
if __name__ == "__main__":
    ...
    model_path = 'model_data/yolov8_m.pth'  # 预训练模型路径
    input_shape = [256, 256]  # 输入图像尺寸
    phi = 'm'  # YOLOv8模型的版本（n表示nano）
    pretrained = False  # 是否使用预训练模型
    mosaic = True  # 是否使用mosaic数据增强
    mosaic_prob = 0.8  # mosaic数据增强的概率
    mixup = True  # 是否使用mixup数据增强
    mixup_prob = 0.8  # mixup数据增强的概率
    special_aug_ratio = 0.9  # 特殊数据增强的比例
    label_smoothing = 0.1  # 标签平滑系数
    Init_Epoch = 0  # 初始训练轮数
    Freeze_Epoch = 30  # 冻结训练的轮数
    Freeze_batch_size = 64  # 冻结训练的批量大小
    UnFreeze_Epoch = 50  # 非冻结训练的总轮数
    Unfreeze_batch_size = 32  # 非冻结训练的批量大小
    ...
    save_dir = 'logs'  # 模型保存目录
    eval_flag = True  # 是否进行评估
    eval_period = 10  # 评估周期
    num_workers = 4  # 数据加载的线程数
    train_annotation_path = '2025_train.txt'  # 训练集标注文件路径
    val_annotation_path = '2025_val.txt'  # 验证集标注文件路径
